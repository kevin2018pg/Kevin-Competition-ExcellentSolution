...
class KFoldProcessor:
    ...
    def _create_examples(cls, data_list, do_predict=False):
        ...
        for data in data_list:
            ...
            # 添加微博情感对象
            examples.append(
                SentimentInputExample(id=id, content=content, label=label))
        return examples
...
def sentiment_convert_examples_to_features(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_ids, token_type_ids = inputs["input_ids"], inputs[
            "token_type_ids"]
        # 遮罩，padding的地方为0，未padding的地方为1     使用encode_plus方法可以得到attention mask
        ...
        features.append(
            SentimentInputFeatures(input_ids, attention_mask, token_type_ids,
                                   label))
    return features
...
def main():
    ...
    if args.k_fold:
        ...
        # 构建维度(data_length,6)
        oof_train = np.zeros(
            (len(all_train_examples), len(k_fold_processor.get_labels())))
        oof_test = np.zeros(
            (len(predict_examples), len(k_fold_processor.get_labels())))
        ...
        for fold_num, (train_idx, dev_idx) in enumerate(
                stratified_folder.split(all_train_examples, all_train_labels),
                start=1):
            logging.info('start fold ' + str(fold_num) + ' training...')
            train_examples, dev_examples = all_train_examples[
                train_idx], all_train_examples[dev_idx]
            if args.use_pseudo_data:
