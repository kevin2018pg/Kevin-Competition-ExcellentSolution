    ...
    for epoch in range(args.num_train_epochs):
        ...
        # 过gradient_accumulation_steps后才将梯度清零，不是每次更新/每过一个batch清空一次梯度，即每gradient_accumulation_steps次更新清空一次
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % 30 == 0:
                    logging.info("EPOCH = [%d/%d] global_step = %d loss = %f",
                                 epoch + 1, args.num_train_epochs, global_step,
                                 logging_loss / 30)
                    logging_loss = 0.0
        ...
        if epoch - best_epoch >= 4:
            logging.info(
                "Long time no improvement, stop train, best epoch = %f",
                best_epoch + 1)
            break
    logging.info("train end, best epoch = {}, best f score = {}".format(
        best_epoch + 1, best_f_score))


def evaluate_and_save_model(args,
                            model,
                            eval_dataset,
                            epoch,
                            global_step,
    ...
    logging.info(
        msg.format(global_step, eval_loss, label_f_score, label_acc, improve))
...
def evaluate(args, model, eval_dataset, is_test=False):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            total_loss += loss * list(
                batch.values())[0].shape[0]  # loss * 样本个数
    ...
    label_f_score = f1_score(y_true=out_label_ids,
                             y_pred=preds,
                             average='macro')
    label_acc = accuracy_score(out_label_ids, preds)
    model.train()
    if is_test:
        report = classification_report(y_true=out_label_ids,
                                       y_pred=preds,
                                       target_names=eval_dataset.get_labels(),
                                       digits=4)
...
def predict(args, model, predict_dataset, processor, pseudo_ratio=None):
    ...
    with open(os.path.join(args.output_dir, output_file),
              'w',
              encoding='utf-8') as f:
    ...
    if pseudo_ratio:
        ...
        with open(os.path.join(args.data_dir, output_file),
                  'w',
                  encoding='utf-8') as f:
